...
#ef=253 #based on Rick Kraus' 2012 paper, stishovite
ef=159.662-154.136 # hemingway 1987.efe=ef*0.03
#Constants
R=8.314 # J/K/mol
G=6.67408*10**(-11) #m^3/kg/s^2
mm=60.08/1000 #kg/mole, molar mass of silica
hot_ent=154.136 # J/(mole*K) quartz at 1800K
hot_ent=hot_ent/mm
ef=ef/mm
efe=ef*0.03
...
##
##plt.figure()
##upb=np.linspace(0,10,100)
##usb=hugoniot(au1,bu1,cu1,du1, upb)
##Pb=betad*upb*usb
##plt.plot(usb, Pb/1000)
##plt.grid()
##plt.show()
...
up=np.linspace(0,15,1000) #Array of particle velocities (km/s)
P=np.linspace(0,10**12,1000) #Pressure array
...
##    ZS1=sFit(P,aq1,bq1,cq1,dq1)
    ZS1=entropy(aq1,bq1,cq1,dq1,P/(10**9))
    #ZSh=entropy(aqh1,bqh1,cqh1,dqh1,P/(10**9))
    #ZSl=entropy(aql1,bql1,cql1,dql1,P/(10**9))
##    ZS1=entropy(aq,bq,cq,dq,eq,P)+entropy_uncert(aqu,bqu,cqu,P)*sp.randn()
    rho=2700+2700*0.03*sp.randn() #Density uncertainty
...
with open('VaporFractions_Quartz_298.txt','w') as f:
