...
Fo_tp_P=5.2*(10**-9)#Pa, 2163C melting point
...
ZP = np.genfromtxt(zfile,dtype=float,delimiter=',',skip_header=261,usecols=0)
ZS = np.genfromtxt(zfile,dtype=float,delimiter=',',skip_header=261,usecols=2)
ZT = np.genfromtxt(zfile,dtype=float,delimiter=',',skip_header=261,usecols=1)
ZPe= np.genfromtxt(zfile,dtype=float,delimiter=',',skip_header=261,usecols=4)
ZTe= np.genfromtxt(zfile,dtype=float,delimiter=',',skip_header=261,usecols=5)
ZSe= np.genfromtxt(zfile,dtype=float,delimiter=',',skip_header=261,usecols=6)
...
#writing to files
with open('VaporFractions_MANEOS_298.txt','w') as f:
    f.write('298K, CM, CMe, IV, IVe, 5, 5e, 10, 10e, 25, 25e, 50, 50e \r\n')
...
with open('VaporFractions_Forsterite_298.txt','w') as f:
    f.write('298K, CM, CMe, IV, IVe, 5, 5e, 10, 10e, 25, 25e, 50, 50e \r\n')
    f.write('atm Pressures')
    f.write(',')
    for i in range(np.size(civ_tp_z_final[:,0])):      
